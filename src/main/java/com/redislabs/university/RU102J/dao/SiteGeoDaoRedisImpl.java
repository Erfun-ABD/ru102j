...
public class SiteGeoDaoRedisImpl implements SiteGeoDao {
    ...
    public Set<Site> findAll() {
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            Set<Response<Map<String, String>>> siteResponses = new HashSet<>(keys.size());
            Pipeline p = jedis.pipelined();

            for (String key : keys) {
                Response<Map<String, String>> siteResponse = p.hgetAll(key);
                siteResponses.add(siteResponse);
            }
            p.sync();
            for (Response<Map<String, String>> siteResponse : siteResponses) {
                Map<String, String> site = siteResponse.get();
                if (!site.isEmpty()) {
       ...
       GeoUnit radiusUnit = query.getRadiusUnit();
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            List<GeoRadiusResponse> radiusResponses = 
                    jedis.georadius(RedisSchema.getSiteGeoKey(), 
            ...
            for (Site site : sites) {
                Response<Double> score = 
                        pipeline.zscore(RedisSchema.getCapacityRankingKey(), 
